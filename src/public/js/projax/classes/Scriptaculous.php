...
class Scriptaculous extends Prototype   {
	...
	function sortable_element($element_id,$options=null)
	{
...
function in_place_editor($field_id,$options,$tag=true) {
	...
	function auto_complete_field($field_id,$options){
		...
		// RSK addition - this was in the docs, but not actually in the code
		if (isset($options['after_update_element']))$js_options['afterUpdateElement']=$options['after_update_element'];
		// JBW addition - this was in the docs, but not actually in the code
	...
	{
	  // RSK addition: allow name different than id
	  $name = isset($tag_options['name']) ? $tag_options['name'] : $object;
	  
		$ret_val=(isset($completion_options['skip_style']))?'':$this->_auto_complete_stylesheet();
		$ret_val.='<input autocomplete="off" id="'.$object.'" name="'.$name.'" size="'.(isset($tag_options['size'])?$tag_options['size']:30).'" type="text" value="'.(isset($tag_options['value'])?$tag_options['value']:'').'" '.(isset($tag_options['class'])?'class = "'.$tag_options['class'].'" ':'').(isset($tag_options['onChange'])?'onChange = "'.$tag_options['onChange'].'" ':'').'/>';
//RSK : added onChange option; really, should handle any options that are passed
