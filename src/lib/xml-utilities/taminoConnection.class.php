  ...
  function xql ($query, $position = NULL, $maxdisplay = NULL) {
   ...
   function getCursor () {
     if ($this->xml) {
       ...
       if ($nl->length) {  $this->position = $nl->item(0)->textContent; }
       $nl = $this->xpath->query("/ino:response/ino:cursor/ino:current/@ino:quantity");
       if ($nl->length) { $this->quantity = $nl->item(0)->textContent; }
   ...
   function initializeXML () {
    ...
    if (!$this->xml) {
     ...
     if ($nl->length) { $this->xq_code = $nl->item(0)->textContent; }
     $nl =  $this->xpath->query("/ino:response/ino:message/ino:messagetext");
     if ($nl->length) { $this->xq_msg = $nl->item(0)->textContent; }
     if ($this->debug) {
