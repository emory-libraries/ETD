...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template name="controlField008-24-27">
    <xsl:variable name="chars">
      <!-- 2009-07-28 la  For ETDS, set first byte to code m for Theses -->
      <xsl:text>m</xsl:text>
      <xsl:for-each select="mods:genre[@authority='marcgt']">
        <!-- 2009-07-28 la  Added code for encyclopedia, corrected term thesis, added codes for legal case and case notes, law report or 
          digest, offprint, and comic strip  -->
        <xsl:choose>
          ...
          <!--2009-07-28  la Don't use for ETDs <xsl:when test=".='thesis'">m</xsl:when>                               -->
          <xsl:when test=".='legal case and case notes'">v</xsl:when>
          <xsl:when test=".='law report or digest'">w</xsl:when>
          <xsl:when test=".='treaty'">z</xsl:when>
          <xsl:when test=".='offprint'">2</xsl:when>
          <xsl:when test=".='comic strip'">6</xsl:when>
        </xsl:choose>
      </xsl:for-each>
    </xsl:variable>
  ...
  <xsl:template match="mods:mods">
    <marc:record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:marc="http://www.loc.gov/MARC21/slim" xsi:schemaLocation="http://www.loc.gov/MARC21/slim http://www.loc.gov/standards/marcxml/schema/MARC21slim.xsd">
      ...
      <marc:controlfield tag="008">
        ...
        <!-- 07-14          -->
        <!-- 07-10 -->
        <!-- ETD simplified selection of date1, added processing of dateIssued without @encoding='marc'  for ETD dates   la 2009-10-02 -->
        <xsl:choose>
          <xsl:when
          ...
          <xsl:when
            test="mods:originInfo/mods:dateIssued[@encoding!='marc' or not(@encoding)]">
        ...
        <xsl:choose>
          <!-- v3 language -->
          <xsl:when
            test="mods:language/mods:languageTerm[@authority='iso639-2b'and @type='code']">
            <xsl:value-of
              select="lower-case(mods:language/mods:languageTerm[@authority='iso639-2b'and@type='code'])"
            />
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>   </xsl:text>
  ...
  <!-- v3 language -->
  <xsl:template match="mods:language/mods:languageTerm[@authority='iso639-2b'and@type='code']">
  ...
  <xsl:template match="mods:note[not(@type='statement of responsibility') and not(@type='admin') and not(@type='partneragencytype')]">
    <xsl:call-template name="datafield">
  ...
  <xsl:template match="mods:originInfo">
    ...
    <!-- ETD added processing for non-@encoding 'marc' dateIssued elements to subfield c  la 2009-10-03-->
...
<xsl:call-template name="datafield">
      ...
      <xsl:with-param name="subfields">
        ...
        <!-- ETD changed this to take 4 character substring when encoding != marc; changed it from a for-each (subfield c is not repeatable) -->
          <xsl:choose>
            <xsl:when test="mods:dateIssued[@keyDate='yes' and @encoding='marc']">
              <marc:subfield code="c">
                <xsl:value-of select="."/>
              </marc:subfield>
            </xsl:when> 
            <xsl:when test="mods:dateIssued[@keyDate='yes']">
              <marc:subfield code="c">
                <xsl:value-of select="substring(mods:dateIssued,1,4)"/>
              </marc:subfield>
            </xsl:when>
            <xsl:when test="mods:dateIssued[not(@keyDate)]">
              <marc:subfield code="c">
...
-->
  ...
  <!--v3 location/url -->
  <!--ETD add test for blank url element and skip those; assign indicator parameters-->
  <xsl:template match="mods:location">
    <xsl:for-each select="mods:url[normalize-space(.)]">
      <xsl:call-template name="datafield">
        <xsl:with-param name="tag">856</xsl:with-param>
        <!-- Add indicators la 2009-09-25 -->
  ...
  <!-- ETD Comment this out because it would duplicate one created from location/url>
  ...
  <!-- v3 role-->
  <!-- added match condition to only use 720 when name doesn't have a type attribute, and if test for content in namePart.  la 2009-10-02 -->
  ...
  <!--la 2009-09-08 ETD add match condition to remove ETD empty name templates, from the cleanMods.xsl  -la -->
  <xsl:template
    match="mods:name[@type='personal'][mods:namePart[@type='given'] != ''
        and mods:namePart[@type='family'] != '' and normalize-space(mods:displayForm) != ','][mods:role/mods:roleTerm[@type='text']='author']">
    <xsl:call-template name="datafield">
        ...
        <marc:subfield code="a">
          <xsl:choose>
            <xsl:when test="mods:namePart[@type='family']">
              <xsl:value-of select="mods:namePart[@type='family']"> </xsl:value-of>
              <xsl:if test="mods:namePart[@type='given']">, <xsl:value-of
  ...
  <!-- v3 role -->
  <xsl:template
    match="mods:name[@type='personal'][mods:role/mods:roleTerm[@type='text']!='author' or not(mods:role)][mods:namePart/text()!=', ']">
    <xsl:call-template name="datafield">
      ...
      <xsl:with-param name="subfields">
        <marc:subfield code="a">
          ...
          <xsl:if test="mods:namePart[@type!='family'and@type!='given']">
            <xsl:value-of select="mods:namePart"/>
          </xsl:if>
        </marc:subfield>
        <!-- v3 termsofAddress -->
  ...
  <xsl:template match="mods:subject[local-name(*[1])='topic' and @authority]">
    <xsl:call-template name="datafield">
  ...
  <!-- For ETDs, comment out - we don't use extension elements this way  la 2009-09-25
