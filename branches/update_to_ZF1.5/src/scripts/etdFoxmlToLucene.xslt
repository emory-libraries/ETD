       ...
       <!-- uncomment it, if you wish, it takes time, even if the foxml has no RIGHTS2 datastream.
            ...
            <!-- This is an example of calling an extension function, see Apache Xalan, may be used for filters.
              ...
              <xsl:template match="mods:mods">
                ...
                <xsl:apply-templates select="mods:identifier[@type='ark']"/>
               ...
               <xsl:value-of select="concat($year, $month, $day)"/>
                ...
                <xsl:template match="mods:name[mods:role/mods:roleTerm = 'author']/mods:affiliation">
                  ...
                  <xsl:template match="mods:subject">
                   ...
                   <!-- administrative note: set if embargo expiration notice email has been sent -->
                   <xsl:template match="mods:note[@ID='embargo_expiration_notice']">
                     <IndexField index="TOKENIZED" store="NO" IFname="embargo_notice">
                       <xsl:value-of select="."/>
                     </IndexField>                     
                   </xsl:template>

                   <!-- embargo duration: 0 days, 6 months, 1 year, 2 years, 6 years  -->
                   <xsl:template match="mods:accessCondition[@type='restrictionOnAccess']">
                     <xsl:variable name="duration">
                       <xsl:value-of select="substring-after(., 'Embargoed for ')"/>
                     </xsl:variable>

                     <xsl:if test="$duration != ''">
                       <IndexField index="UN_TOKENIZED" store="NO" IFname="embargo_duration">
                   ...
                   <xsl:template match="mods:identifier[@type='ark']">
                     <xsl:if test=". != ''">
                       <IndexField index="UN_TOKENIZED" store="YES" IFname="ark">
                         <xsl:value-of select="."/>
                       </IndexField>
                     </xsl:if>
                   </xsl:template>
