...
try { // Does the pid collection object exist in fedora?
...
if (!$programs) { // The fedora object does not exist, so create 
  $logger->notice("Top-level collection {$config->programs_collection->label} ($pid) not found in Fedora; creating it");
  $new_collection  = new foxmlPrograms(); // no parameter indicates create collection.
  
  // Set the initial values for the fedora collection
  $new_collection->pid = $config->programs_collection->pid;
  $new_collection->label = $config->programs_collection->label;
  $new_collection->owner = $config->etdOwner;  
  
  // Add a model in the RELS-EXT datastream (Subject/Predicate/Object)
  $new_collection->setContentModel($config->programs_collection->model_object);
    
  if ($opts->noact) {   
      $logger->info("Ingesting {$config->programs_collection->label} ($pid) into Fedora (simulated)");
      $logger->debug($new_collection->saveXML());
  } else {
    $success = $new_collection->ingest("creating LSDI collection object");
    if ($success) {     
      $logger->info("Successfully ingested {$config->programs_collection->label} ($pid) into Fedora");
      // fedora object has been create, now load it.
      $programs = new foxmlPrograms("#programs");
    } else {     
      $logger->err("Failed to ingest {$config->programs_collection->label} ($pid) into Fedora");
    }
  }
}

if (!$programs) {
  $logger->err("Failed to create the  {$config->programs_collection->label} ($pid) in Fedora");
  return;
}
$skos = $programs->skos;
